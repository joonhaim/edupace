...
void setupDisplay(){
  /*Background*/
  lv_obj_t * scr = main_scr;               // get the active screen
  lv_obj_set_style_bg_color(scr, lv_color_hex(0x03989e), LV_PART_MAIN); 

  /*Main container*/
  cont = lv_obj_create(scr);
  lv_obj_set_size(cont, 450, 770);             
  lv_obj_set_style_bg_color(cont, lv_color_hex(0x808080), LV_PART_MAIN); // medium gray at first
  lv_obj_center(cont);                         
  lv_obj_set_style_radius(cont, 20, LV_PART_MAIN); // round the corners
  ...
  /*
  //EduPace logo
  lv_obj_t * edupace_small_img_obj = lv_img_create(cont);
  lv_img_set_src(edupace_small_img_obj, &edupace_logo_small_img);  // Replace with actual image variable
  lv_obj_align(edupace_small_img_obj, LV_ALIGN_CENTER, -10, 350);
  */
  ...
  lv_obj_set_size(key_btn, 110, 80); 
  lv_obj_align(key_btn, LV_ALIGN_BOTTOM_RIGHT, -5, -5); 
  ...
  lv_obj_set_style_radius(key_btn, 20, LV_PART_MAIN); // round the corners 
  ...
  /*Power Button*/
  power_btn = lv_btn_create(cont);
  lv_obj_set_size(power_btn, 110, 80); 
  lv_obj_align(power_btn, LV_ALIGN_BOTTOM_LEFT, 5, -5); 
  lv_obj_set_style_bg_color(power_btn, lv_color_hex(0x000000), LV_PART_MAIN); 
  lv_obj_set_style_border_color(power_btn, lv_color_white(), LV_PART_MAIN);
  lv_obj_set_style_border_width(power_btn, 6, LV_PART_MAIN); // 6 px thick
  lv_obj_set_style_radius(power_btn, 20, LV_PART_MAIN); // round the corners 

  /*Icon inside power button*/
  lv_obj_t * power_icon = lv_label_create(power_btn);
  lv_label_set_text(power_icon, LV_SYMBOL_POWER);   
  lv_obj_set_style_text_color(power_icon, lv_color_white(), LV_PART_MAIN); 
  lv_obj_set_style_text_font(power_icon, &lv_font_montserrat_40, LV_PART_MAIN);
  lv_obj_center(power_icon);

  //Click event on power button
  lv_obj_add_event_cb(power_btn, power_button_event_cb, LV_EVENT_ALL, NULL);
  ...
  lv_obj_align(battery, LV_ALIGN_BOTTOM_LEFT, 130, 0); 

  /*Number 1*/
  top_number = lv_label_create(cont);         // create label inside container
  lv_label_set_text(top_number, "--");                    
  lv_obj_set_style_text_color(top_number, lv_color_black(), LV_PART_MAIN); 
  lv_obj_set_style_text_font(top_number, &digital, LV_PART_MAIN); 
  lv_obj_align(top_number, LV_ALIGN_TOP_RIGHT, -15, 90);      

  /*Number 2*/
  middle_number = lv_label_create(cont);        
  lv_label_set_text(middle_number, "--");                   
  lv_obj_set_style_text_color(middle_number, lv_color_black(), LV_PART_MAIN); 
  lv_obj_set_style_text_font(middle_number, &digital, LV_PART_MAIN); 
  lv_obj_align(middle_number, LV_ALIGN_TOP_RIGHT, -15, 305);       

  /*Number 3*/
  bottom_number = lv_label_create(cont);         
  lv_label_set_text(bottom_number, "--");                   
  lv_obj_set_style_text_color(bottom_number, lv_color_black(), LV_PART_MAIN); 
  lv_obj_set_style_text_font(bottom_number, &digital, LV_PART_MAIN); 
  lv_obj_align(bottom_number, LV_ALIGN_TOP_RIGHT, -15, 520);       

  /*Label 1: Pace*/
  first_label = lv_label_create(cont);
  lv_label_set_text(first_label, "Rate"); 
  ...
  lv_label_set_text(second_label, "Output"); 
  ...
  lv_label_set_text(first_unit, "ppm"); 
  ...
  lv_label_set_text(second_unit, "mA"); 
